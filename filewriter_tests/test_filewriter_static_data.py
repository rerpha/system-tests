...
def test_static_data_reaches_file(docker_compose_fw):
    ...
    send_writer_command(os.path.join(
        "filewriter_tests", "commands", "static-data-add.json"),
        producer,
    ...
    sleep(10)
    # Stop file writing
    send_writer_command(os.path.join("filewriter_tests", "commands", "static-data-stop.json"), producer)
    sleep(10)
    send_writer_command(os.path.join("filewriter_tests", "commands", "writer-exit.json"), producer)
    producer.flush()

    filepath = os.path.join("filewriter_tests", "output-files", "output_file_static.nxs")
